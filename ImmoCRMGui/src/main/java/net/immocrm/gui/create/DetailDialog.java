...
public class DetailDialog extends AbstractCreationDialog<Immobilie> {

	private final ImmobilieFormController ctrl;


	public DetailDialog(AssistentDialogFrame dialogFrame) {
		super(dialogFrame);
		dialogFrame.addButtons(ButtonData.BACK_PREVIOUS, ButtonData.NEXT_FORWARD, ButtonData.CANCEL_CLOSE);
		dialogFrame.setHeader("Immobilie");
	...
	protected DialogSaveValidator getValidator() {
		...
		return event -> {
			...
			return validate(immobilie.getAddress());
		};
	}

	private boolean validate(Address address) {
		if (address.isEmpty()) {
			AlertProvider.alertError("Bitte eine Adresse eingeben!\nAlle anderen Angaben können später erfolgen.");
			return false;
		}
		ValidationIssues issues = address.validate();
		if (issues.hasIssues()) {
